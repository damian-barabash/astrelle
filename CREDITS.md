# Photo credits

## Clay-type photos (`public/assets/img/clay-*.webp`)

- **clay-terracotta** — “Gasteria Exelsa in Clay Pot” by **Chris Hunkeler**,
  licensed **CC BY-SA 2.0** — https://www.flickr.com/photos/14913305@N00/20638744944
  (https://creativecommons.org/licenses/by-sa/2.0/). Attribution required.
- **clay-stoneware** — Unsplash, by *cafeconcetto*.
- **clay-porcelain** — Unsplash, by *Tamara Harhai*.
- **clay-white** — Unsplash, by *Tom Crew*.

## Section / hero photos (`public/assets/img/photo-*.webp`)

All from **Unsplash** (Unsplash License — free for commercial use, attribution appreciated):
Anne Nygård · cafeconcetto · Jocelyn Morales · Pew Nguyen · Tamara Harhai ·
Taylor Heery · Tom Crew.

## Goat mascots (`public/assets/maskot/goat-01..18.{png,webp}`)

Sliced from an AI-generated icon sheet (OpenArt) into 18 individual transparent
icons by `scripts/goats.mjs` (recoloured to the brand sage). Source sheet kept
locally in `media-src/maskot/pack.jpg` (outside git).

Original full-resolution sources are kept locally in `media-src/` (outside git).

## Fonts

- **Geologica** (headlines) and **Onest** (text) — Google Fonts, SIL Open Font License 1.1.
  Loaded from `fonts.googleapis.com` (see `index.html`).
